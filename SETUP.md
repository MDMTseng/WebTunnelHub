...
本文说明如何从空白环境搭好一条完整链路：**本机**运行一个或多个 Hello World（或其它 Web 服务），通过 **SSH 反向隧道** 接到 **EC2**，再由 **Caddy** 在 `**https://你的域名:1080`** 上对外提供 **HTTPS**，并支持 **路径分流**（Hub：`/应用名/`）。
...
```
浏览器 --HTTPS--> EC2:1080 (Caddy：TLS + 按路径反代)
        |                |
        |                +--> 127.0.0.1:10080 ──SSH──> 本机 :8080   （根站 /）
        |                +--> 127.0.0.1:<自动端口> ──SSH──> 本机 :<各应用端口>  （/CoolApp/、/Hello2/ …）
        v
https://域名:1080/          → 默认隧道 10080
https://域名:1080/CoolApp/  → 该应用专用回环端口（由应用名稳定算出）
```
...
- **子路径应用**：每个应用 `**./hub-register.sh 应用名`**（EC2 上写 Caddy 片段）+ `**./hub-tunnel.sh --port 本地端口 应用名**`（长期开着）。
...
| 域名          | 例如 `db.xception.tech`，**A 记录**指向 EC2 **公网 IPv4** |
...
| `hub-register.sh`       | 在 EC2 注册 `**/应用名/**` 路由；**同名已存在则失败退出码 2** 且不改动配置；`**--force`** 可覆盖                                                                                                                                                |
...
- `**db.xception.tech:1080**`：对外 HTTPS；浏览器访问 `**https://db.xception.tech:1080/**` 与各 `**/应用名/**`。
- `**import .../hub-routes/*.caddy**`：每个应用一个片段（由 `**hub-register.sh**` 生成）；目录内需至少有一个匹配文件（脚本会创建 `**_keep.caddy**` 占位）。
- `**handle { ... 10080 }**`：根路径走默认 SSH 隧道（本机 `**./hub-tunnel.sh**` 无应用名）。

**仅根站、不要 Hub 时**可简化为单块 `**reverse_proxy 127.0.0.1:10080`**（无 `**import**`）；加新应用前再改回 Hub 布局。
...
3. **公网 HTTPS**：
  ```bash
   ...
   curl -sS https://db.xception.tech:1080/Hello2/ | head
  ```
   根站与各 `**/应用名/**` 均应为 **200** 且返回预期 HTML。
...
**5. Hub 路径 404**  
先执行 `**./hub-register.sh 应用名`** 并成功 `**reload**` Caddy，再 `**./hub-tunnel.sh --port … 应用名**`；主 `**Caddyfile**` 须含 `**import /etc/caddy/hub-routes/*.caddy**`。
...
## 十、多应用 Hub（`https://域名:1080/应用名/`）

> **与第六节的关系**：第六节给出「根站 + Hello2」的**逐步命令**；本节归纳 **Hub 规则**与 **Caddy 要点**，便于查阅。

把 EC2 当成入口：**每个本地 Web 服务**一条 SSH 反向隧道，Caddy 按 **路径前缀**转发到不同 EC2 回环端口。

### 10.1 流量关系

```
浏览器  https://db.xception.tech:1080/CoolApp/...
              → Caddy（TLS）
              → 127.0.0.1:<该应用专用端口>
...
### 10.2 EC2：主 Caddyfile 增加 import

将站点块改为与仓库 `**Caddyfile.ec2.example**` 一致（在 `**db.xception.tech:1080 {**` 内先 `**import**`，再 `**handle { reverse_proxy 127.0.0.1:10080 }**` 作为默认站）：
...
应用名只能包含 **字母、数字、下划线、连字符**，且不能以连字符开头（与 URL 路径一致）。
...
- `**https://db.xception.tech:1080/CoolApp/**`、`**https://db.xception.tech:1080/Hello2/**` 等（建议带 **尾部斜杠**；访问 `**/CoolApp`** 会 **301** 到 `**/CoolApp/`**）。

### 10.6 前端 / 资源路径说明

Caddy 会把 `**/CoolApp**` 前缀 **剥掉** 再转给上游，因此上游看到的请求路径以 `**/`** 为根。若框架写死了绝对路径 `**/assets/...**`，在子路径下可能 404，需在应用里配置 **base path / assetPrefix**（如 `**/CoolApp`**），或使用 **根路径** 部署方式。
...
2. 浏览器或另一台机器：`https://你的域名:1080/`、`**https://你的域名:1080/Hello2/`** 应为 **200**。
...
8. 用 `**curl**` 或浏览器验证 `**https://域名:1080/**` 与 `**https://域名:1080/应用名/**`（见第七节）。
